...
@with_attrs(nl_names=["n_term_angles", "bulk_angles", "c_term_angles", "angles"])
def standard_angles(
    topology: md.Topology, separate_termini: bool = True, **kwargs
) -> Union[List[Tuple[str, int, torch.Tensor]], Tuple[str, int, torch.Tensor]]:
    mlcg_top = Topology.from_mdtraj(topology)
...
@with_attrs(nl_names=[f"{res}_phi" for res in all_residues])
def phi(
    topology: md.Topology, **kwargs
) -> Union[List[Tuple[str, int, torch.Tensor]], Tuple[str, int, torch.Tensor]]:
...
@with_attrs(nl_names=[f"{res}_psi" for res in all_residues])
def psi(
    topology: md.Topology, **kwargs
) -> Union[List[Tuple[str, int, torch.Tensor]], Tuple[str, int, torch.Tensor]]:
    dihedral_dict = get_dihedral_groups(
...
@with_attrs(nl_names=["pro_omega", "non_pro_omega"])
def omega(topology: md.Topology, **kwargs) -> List[Tuple[str, int, torch.Tensor]]:
    dihedral_dict = get_dihedral_groups(
        topology,
